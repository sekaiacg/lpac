...
static int applet_main(int argc, char **argv)
{
	...
	int name_bytes_size = 0;

    if (argc < 2)
    {
        printf("Usage: %s [iccid] [new_name] [name_bytes_size]\n", argv[0]);
        printf("\t[new_name]: optional\n");
		printf("\t[name_bytes_size]: optional, If provided, the nickname will be decoded using base64.\n");
        return -1;
    }
    ...
    if (argc > 3)
    {
        new_name_decode = malloc(euicc_base64_decode_len(argv[2]));
        if (new_name_decode) {
            int len = euicc_base64_decode(new_name_decode, argv[2]);
    ...
    {
        new_name = argv[2];
        name_bytes_size = strlen(new_name);
    }
    else
    {
        new_name = "";
    ...
    if ((ret = es10c_set_nickname(&euicc_ctx, iccid, new_name, name_bytes_size)))
    {
        const char *reason;
        switch (ret)
        {
