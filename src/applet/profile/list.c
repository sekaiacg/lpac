...
char *handleProfileNicknameBase64(cJSON *jprofile, struct es10c_profile_info_list *profile) {
    ...
    const int profileNicknameSize = profile->profileNicknameSize;
    char *b64Nickname = NULL;
    if (profileNickname) {
        int size = euicc_base64_encode_len(profileNicknameSize);
        b64Nickname = (char *)malloc(size);
        if (b64Nickname) {
            memset(b64Nickname, 0, size);
            euicc_base64_encode(b64Nickname, (const unsigned char *)profileNickname, profileNicknameSize);
        }
    }
    return b64Nickname;
}
